...
if DrawSprite then
	...
	function visual.drawprism( transform, radius, depth, rotation, sides, info )
		...
		for v = 0, 360, 360 / sides do
			points[iteration] = TransformToParentPoint( transform, Vec( sin( v * degreeToRadian + rotation ) * radius, 0,
			                                                            cos( v * degreeToRadian + rotation ) * radius ) )
			points[iteration + 1] = TransformToParentPoint( transform, Vec( sin( v * degreeToRadian + rotation ) * radius,
			                                                                -depth, cos( v * degreeToRadian + rotation ) * radius ) )
			if iteration > 2 then
				DrawFunction( points[iteration], points[iteration + 1], r, g, b, a )
